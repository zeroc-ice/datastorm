...
main(int argc, char* argv[])
{
    ...
    {
        {
            ...
            reader.onSamples([&p](const vector<Sample<string, string>>& samples)
            {
                test(samples.size() == 1);
                p.set_value();
            }, [](const Sample<string, string>& sample)
            {
                test(false);
            });
            p.get_future().wait();
        ...
        }
        {
            ...
            promise<void> p;
            reader.onSamples([](const vector<Sample<string, string>>& samples)
            {
                test(false);
            }, [&p](const Sample<string, string>& sample)
            {
                p.set_value();
            });
            readers.update(2);
    ...
    // onConnectedKeys
    {
        ...
        }
        {
            ...
            reader.onConnectedKeys([&p1](vector<string> keys)
            {
    ...
    // onConnected
    {
        ...
        }
        {
            auto reader = makeSingleKeyReader(topic, "elem3", "", config);
            promise<bool> p1, p2, p3;
            reader.onConnectedWriters([&p1](vector<string> writers)
            {
